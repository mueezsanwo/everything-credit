...
import { User, ArrowLeft, Mail, Phone, Briefcase, Building2, Landmark, Shield, CreditCard, TrendingUp, Calendar, LogOut, CheckCircle, Loader2, XCircle } from 'lucide-react';
import Link from 'next/link';
import { getUserProfile, cancelMandate } from '@/lib';
...
export default function Profile() {
  ...
  const handleCancelMandate = async () => {
    if (!confirm('Are you sure you want to cancel your mandate? This will affect your ability to make credit purchases.')) {
      return;
    }

    setCancellingMandate(true);
    try {
      const response = await cancelMandate();
      showToast(response.message, 'success');
      
      // Refresh profile to get updated mandate status
      const profileResponse = await getUserProfile();
      if (profileResponse.success && profileResponse.data) {
        setUserData(profileResponse.data);
      }
    } catch (error: any) {
      showToast(error?.message || 'Failed to cancel mandate', 'error');
    } finally {
        ...
        <main className="max-w-5xl mx-auto px-4 py-4">
          ...
          {activeTab === 'credit' && (
            <div className="space-y-4">
              ...
              {/* Cancel Mandate Section */}
              {userData.hasMandateCreated && userData.mandateStatus === 'ACTIVE' && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <XCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" />
                    <div className="flex-1">
                      <h4 className="font-semibold text-red-900 mb-2 text-sm">Cancel Direct Debit Mandate</h4>
                      <p className="text-xs text-red-700 mb-3">
                        Canceling your mandate will stop automatic deductions from your account. You won't be able to make new credit purchases until you create a new mandate.
                      </p>
                      <button
