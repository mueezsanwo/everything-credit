...
import { shouldTriggerDelivery, areAllPaymentsCompleted } from '@/lib/payments/scheduler';
...
export async function POST(request: Request) {
  try {
    ...
    // Verify signature
    if (!signature || !verifyWebhookSignature(body.request_ref, signature, APP_SECRET)) {
...
async function handleTransactionNotification(details: any, webhookId: any) {
  ...
  if (transaction_type === 'collect' && meta?.event_type === 'debit') {
    await handlePaymentCollection(transaction_ref, status, amount, meta, webhookId);
  }
...
async function checkAndUpdatePurchase(purchaseId: any) {
  try {
    ...
    } else if (percentPaid >= 50 && purchase.deliveryStatus === 'pending_payment') {
      await Purchase.updateOne(
        { _id: purchaseId },
