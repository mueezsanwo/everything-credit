...
export async function GET(request: Request) {
  try {
    ...
    const usersWithCredit = await User.countDocuments({ hasAccessedCredit: true });
    ...
    const totalOutstanding = allPendingPayments.reduce((sum, payment) => sum + payment.amount, 0);
