...
export async function GET(request: Request) {
  try {
    ...
    const loansWithPayments = loans.map((loan) => {
      const loanPayments = allPayments.filter(
        (p) => p.loanId && p.loanId.toString() === loan._id.toString(),
      );
    ...
    // Combine all items
    const allItems = [...loansWithPayments, ...purchasesWithPayments].sort(
    ...
    const upcomingPayments = allPayments
      ...
      .map((p) => {
        ...
        const parentLoan = loans.find(
          (l) => l._id.toString() === p.loanId?.toString(),
        );
        const parentPurchase = purchases.find(
          (pur) => pur._id.toString() === p.purchaseId?.toString(),
        );
        ...
        return {
          ...
          itemName: parentLoan
            ? parentLoan.name
            : parentPurchase
              ? parentPurchase.items.map((i) => i.name).join(", ")
              : "Unknown",
        };
      });
